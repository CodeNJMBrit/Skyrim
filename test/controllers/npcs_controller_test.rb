...
class NpcsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create npc" do
    assert_difference('Npc.count') do
      post npcs_url, params: { npc: { defeated_by_char: @npc.defeated_by_char, description: @npc.description, level: @npc.level, name: @npc.name } }
    end
  ...
  test "should update npc" do
    patch npc_url(@npc), params: { npc: { defeated_by_char: @npc.defeated_by_char, description: @npc.description, level: @npc.level, name: @npc.name } }
